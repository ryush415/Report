...
# }
# t.date <- strftime(df.s$collect_date[1], format = "%Y년 %m월 %d일")
```
...
# df.w <- read_csv(paste0("/data/home/1111678/Data/Weather/Boramae/Boramae_weather_", 
#                         strftime(df.s$collect_date[1], format = "%Y%m%d"), ".csv"))
...
df.t1$chw.dt <- df.t1$ch.cool / df.t1$chwp.flow * 60 / 4.186 # 전체 부하와 전체 유량으로 평균 냉수 입출구 온도차 계산
df.t1$ch.usRT <- (df.t1$ch.cool / 4.186 * 3600) / 3024 # usRT로 환산
